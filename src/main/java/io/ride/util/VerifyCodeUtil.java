...
public class VerifyCodeUtil {
    // 验证码值
    private StringBuilder codeValue;
    ...
    private VerifyCodeStyle style;

    public VerifyCodeUtil(VerifyCodeStyle style) {
        this.style = style;
    ...
    public BufferedImage getVCImg() {
        ...
        switch (style.getCodeType()) {
            case VerifyCodeStyle.EN_CODE:
                words = this.dictEn;
                break;
            case VerifyCodeStyle.CH_CODE:
                words = this.dictZh;
                break;
            case VerifyCodeStyle.MIXED_CODE:
                words = this.dictMIXD;
                break;
        }
        ...
        for (int i = 0; i < style.getWordLen(); i++) {
            ...
            this.codeValue.append(str);

            // 设置字体颜色
            if (style.isColorful) {
                g.setColor(new Color(this.color[random.nextInt(color.length)]));
            }
    ...
    public void outputImg(HttpServletResponse response, BufferedImage vcImg) {
     ...
     * 内部类 验证码风格
     */
    public static class VerifyCodeStyle {
        public static final int EN_CODE = 0;        // 英文验证码
        public static final int CH_CODE = 1;        // 中文验证码
        public static final int MIXED_CODE = 2;     // 中英混合验证码

        private int width = 100;                    // 验证码宽
        private int height = 40;                    // 验证码高
        private int codeType = EN_CODE;             // 验证码类型
        private Color wordColor = Color.BLUE;       // 验证码字体颜色
        private boolean isColorful = true;          // 验证码是否为彩色
        private Color bgColor = Color.WHITE;        // 验证码背景颜色
        private Color lineColor = wordColor;        // 验证码干扰线颜色
        private int lineNum = 4;                    // 验证码干扰线数量
        private float lineStroke = 1.0F;            // 验证码干扰线粗细
        private boolean isRotate = true;            // 是否旋转
        private int wordLen = 4;                    // 验证码字数
        private Font fontStyle = new Font("MS Song", Font.BOLD, 25);        // 验证码字体

        public VerifyCodeStyle() {
        }

        public VerifyCodeStyle(int width, int height, int codeType, Color wordColor, boolean isColorful,
                               Color bgColor, Color lineColor, int lineNum, float lineStroke, boolean isRotate,
                               int wordLen, Font fontStyle) {
            this.width = width;
            this.height = height;
